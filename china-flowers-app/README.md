...
| 问题 | 原因 | 修复 |
|------|------|------|
| 季节筛选后 3D 地球标注视觉上无变化 | `three-globe` 默认 `pointsTransitionDuration=1000ms`，新标注从地球内部动画上升，过渡期间不可见 | 初始化时设置 `.pointsTransitionDuration(0)` 使标注立即出现；访问器改为仅初始化一次 |
| 部分花卉图片加载失败（HTTP 404） | Wikimedia Commons 不生成超过原图宽度的缩略图，7 个图片原始尺寸 < 960px | 将这 7 个 URL 的缩略图尺寸从 960px 改为 640px |
| 图片加载失败后兜底图依赖外部网络 | 使用 Unsplash 作为 fallback，网络不稳定时永久显示加载中 | 改用内联 SVG 数据 URI，基于花卉代表色本地生成占位图，零网络依赖 |
